//정수 배열을 항상 증가 순으로 유지하는 SortedArray 클래스를 작성
#include <iostream>
using namespace std;
class SortedArray {
	int size; // 현배 배열의 크기
...
void SortedArray::sort() {				//오름차순으로 정렬
	for (int i = 0; i < size - 1; ++i) {		// 첫번째와 두번째, 마지막에서 두번째와 마지막의 교환 유무 확인
		for (int j = 1; j < size - i; ++j) {    // 반복문을 통해 오름차순으로 정렬
			if (p[j - 1] > p[j]) {
				int temp = p[j - 1];//위치 교환
				p[j - 1] = p[j];
				p[j] = temp;
			}
		}
