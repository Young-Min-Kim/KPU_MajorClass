...
public class ch07_05 {
	public static void main(String[] args) {
		Student[] student = new Student[4]; // 배열로 학생 클래스 생성 4개
		ArrayList<Student> manage = new ArrayList<Student>(); // 어레이리스트
		Scanner sc = new Scanner(System.in);
		System.out.println("학생 이름, 학과, 학번, 학점평균 입력하세요.");
		for(int i=0; i<student.length; i++) {
			System.out.print(">> ");
			String text = sc.nextLine();
			StringTokenizer st = new StringTokenizer(text, ","); //로 구별
			String name = st.nextToken().trim(); //trim() 은 공백 제거를 위함
...
class Student {
	...
	public void find_student(String name) { //이름으로 검색
